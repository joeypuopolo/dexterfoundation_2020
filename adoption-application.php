...
if( isset($_POST['Form_Name']) && $_POST['Form_Name'] == 'Application_Submitted' ) {
  ...
  $Favorite = "No";
  $Specific_Dog = filter_input(INPUT_POST, "Specific_dogs_name");
  if ( $Specific_Dog == null ) {
    $Specific_Dog = 0;
  }
  if ( $Lease == null ) {
    $Lease = "Blank";
  ...
  if ( $Form_Fields == null || $Date_Entered == null || $Active == null || $Lease == null ) {
    ...
    $query = 'INSERT INTO Applications(ID, Date_Entered, Form_Fields, Active, Lease, Name, Favorite, Specific_Dog) VALUES(:ID, :Date_Entered, :Form_Fields, :Active, :Lease, :Name, :Favorite, :Specific_Dog)';
    ...
    $stm->bindValue(':Specific_Dog', $Specific_Dog);
    ...
    if(!$execute_success){
      ...
      $Specific_Dog_Link = ( $Specific_Dog === "Blank" ) ? "<i>No specific dog specified</i>" : "<a href='https://www.dexterfoundation.com/our-dogs/?pet=$Specific_Dog'>This is the dog I'm interested in!</a>";
      $leaseCopy = ( $Lease === "Blank" ) ? "<i>No lease submitted</i>" : "<a href='https://www.dexterfoundation.com/browser/leases/$Lease'>View Lease [+]</a>";
      $emailBody ="
      <html>
        <head></head>
        <body>
          $Specific_Dog_Link<br><br>
          <a style='font-family: 'Segoe UI', Helvetica, sans-serif;font-size:30px' href='https://www.dexterfoundation.com/applications/?id=$FormId'>
      ...
      ";
      mail("contactus@dexterfoundation.com", "Application - ".$Name, $emailBody, $headers);
...
<head>
  ...
  <style>
    ...
    .pet-select .make-a-selection.active {
      display: block;
      position: fixed;
      width: 95vw;
      height: auto;
    ...
    .item-selector {
      background-color: hsl(195.2, 63.2%, 81%);
      padding: 10px;
      margin: 1em 0;
      display: flex;
      flex-wrap: wrap;
    ...
    .item-selector li, .selected-item-wrapper {
    ...
    .selected-item-wrapper span {
      display: block;
      max-width: 150px;
    }
    .item-selector li:hover {
      background-color: hsl(195.2, 63.2%, 81%);
      color: hsl(251.3, 15.4%, 20.4%);
    }
    .item-selector li img, .selected-item-wrapper img {
      max-width: 100px;
      display: block;
      margin: auto;
    }
    .pet-select .make-a-selection {
      display: none;
    }
    .pet-select .btn.select-btn {
      display: table;
      margin: auto;
    }
    span.cancel {
    ...
    @media screen and (max-width: 500px) {
      ...
      .item-selector .selection-label > span {
        display: block;
        text-align: center;
        margin: 0 0 1em;
      }
...
<body data-pagename="Adoption Application">
  ...
  <main>
        ...
        <header>
          <h1>
            Thank You!
              <small>
                <?php 
                  if ( $Status != "" ) {
                    $redirect = "
                    <script>setTimeout(function(){window.location.replace('https://www.dexterfoundation.com');}, 5000)</script>
                    ";
            ...
            <div class="btn-wrapper flex">
              <a href="https://www.dexterfoundation.com" class="btn btn-orange">Go Home</a>
    ...
    <section class="adoption-application-form content form">
      <div class="wrapper">
        ...
        <div class="content">
          <p><strong>Thank you for you interest in adopting/fostering a dog from The Dexter Foundation!</strong></p>
      ...
      <form method="POST" action="https://www.dexterfoundation.com/adoption-application/" enctype="multipart/form-data">
        <fieldset>
          <ul>
            <li class="half">
              <div class="field">
            ...
            <li class="half">
              <div class="field selector pet-select">
                <input class="hide" value="" type="text" name="Specific_dogs_name">
                <?php
                    require_once($_SERVER['DOCUMENT_ROOT'].'/db_connect.php');
                    $query_dogs = "SELECT * FROM Dog_Listings WHERE Active = 'Yes' ORDER BY Sequence ASC, ID ASC";
                    $dog_statement = $db->prepare($query_dogs);
                    $dog_statement->execute();
                    $dogs = $dog_statement->fetchAll();
                    $dog_statement->closeCursor();
                  ?>
                <div class="item-selector" data-selection="<?php if (isset($_GET['pet'])) { echo $_GET['pet']; } ?>" data-name="Specific_dogs_name">
                  
                  <div class="selection-label">
                    <span>Which dog are you interested in?</span>
                    <div class="selected-item">
                      <div class="selected-item-wrapper hide"></div>
                      <a href="javascript:void(0)" class="btn select-btn">Select</a>
                    </div>
                  </div>
                  <div class="make-a-selection">
                    <div class="cancel-wrapper">
                      <span class="cancel">&times;<i>cancel</i></span>
                    </div>
                    <ul class="item-selector-list">
                      <?php foreach($dogs as $dog) : ?>
                        <li class="item" data-id="<?php echo $dog['ID']; ?>">
                          <div class="wrapper">
            ...
            <li class="full hide">
              <input type="text" name="dog_name" value="">
            </li>
            <li class="half">
              <div class="field">
  ...
  <?php include($_SERVER['DOCUMENT_ROOT']."/modules/sponsors.php"); ?>
</main>
  <footer>
  ...
  <script type="text/javascript">
    function dogSelection() {
      // opens the selector
      $('.pet-select .item-selector .select-btn').on('click', function(){
        $('.pet-select .make-a-selection').addClass('active');
      });

      // closes the selector
      $('.pet-select .item-selector .cancel').on('click', function(){
        $('.pet-select .make-a-selection').removeClass('active');
      });
      
      // click event for selecting the dog
      $('.pet-select .item-selector-list .item').on('click', function(){
        $('.pet-select input[name=Specific_dogs_name').val($(this).data('id'));
        $('input[name=dog_name]').val($(this).find('span.name').html());
        $('.pet-select .item-selector .selected-item-wrapper').html( $(this).find('.wrapper').html() );
        $('.selected-item-wrapper.hide').removeClass('hide');
        $('.pet-select .make-a-selection').removeClass('active');
      });
    }
    dogSelection();
